...
(function (root, factory) {
  ...
  // base = costo de referencia; build = ticks de obra; rentY = renta anual % del valor;
  // cyc = sensibilidad al ciclo macro; boost = eleva el landPrice0 del estado al completarse (megaproyectos)
  const RE_TYPES = {
    house:      { name: 'Casa / duplex',            base: 250000, build: 6,   rentY: 0.085, cat: 'residential', cyc: 0.8 },
    apartment:  { name: 'Edificio de departamentos',base: 2.5e6,  build: 16,  rentY: 0.095, cat: 'residential', cyc: 0.9 },
    complex:    { name: 'Complejo residencial',     base: 12e6,   build: 30,  rentY: 0.10,  cat: 'residential', cyc: 1.0 },
    luxtower:   { name: 'Rascacielos de lujo',      base: 60e6,   build: 52,  rentY: 0.11,  cat: 'residential', cyc: 1.5 },
    shop:       { name: 'Local comercial',          base: 400000, build: 8,   rentY: 0.095, cat: 'commercial',  cyc: 1.0 },
    stripmall:  { name: 'Strip mall',               base: 3e6,    build: 20,  rentY: 0.105, cat: 'commercial',  cyc: 1.1 },
    mall:       { name: 'Shopping mall',            base: 40e6,   build: 60,  rentY: 0.12,  cat: 'commercial',  cyc: 1.4 },
    office:     { name: 'Torre de oficinas',        base: 35e6,   build: 50,  rentY: 0.105, cat: 'commercial',  cyc: 1.7 },
    warehouse:  { name: 'Galpon logistico',         base: 1.2e6,  build: 10,  rentY: 0.095, cat: 'industrial',  cyc: 0.9 },
    indpark:    { name: 'Parque industrial',        base: 15e6,   build: 36,  rentY: 0.105, cat: 'industrial',  cyc: 1.0 },
    fulfillment:{ name: 'Centro de distribucion',   base: 25e6,   build: 40,  rentY: 0.115, cat: 'industrial',  cyc: 1.1 },
    hotel:      { name: 'Hotel boutique',           base: 8e6,    build: 26,  rentY: 0.12,  cat: 'hospitality', cyc: 1.8 },
    resort:     { name: 'Resort turistico',         base: 50e6,   build: 70,  rentY: 0.14,  cat: 'hospitality', cyc: 2.2 },
    mixeduse:   { name: 'Megaproyecto de uso mixto',base: 150e6,  build: 90,  rentY: 0.125, cat: 'mega',        cyc: 1.3, boost: 0.05 },
    district:   { name: 'Urbanizacion planificada', base: 400e6,  build: 120, rentY: 0.135, cat: 'mega',        cyc: 1.4, boost: 0.12 },
  };
  const RE_ALIAS = { residential: 'house', commercial: 'shop', industrial: 'warehouse', land: 'house' };
  ...
  function createInitialState(config) {
    ...
    const state = {
      ...
      event: { active: null, cooldown: 4 }, pendingEvents: [], football: null, nations: buildNations(),
  ...
  function recomputeNetWorth(state) {
    ...
    if (state.football) nw += footballValue(state.football);
  ...
  function tick(state) {
    ...
    stepCredit(state);                 // 9
    maybeEvent(state);                 // 10
    stepFootball(state);               // 10b (club de futbol)
    if (state.tick % 4 === 0) stepNations(state); // 10c (naciones, cada 4 ticks)
    stepClose(state);                  // 11
    return state;
  }
  ...
  function stepCapitalMarkets(state) {
    ...
    // inmuebles: fases de obra; valor sigue landPrice + inflacion + ciclo; ocupacion por oferta/demanda de la categoria
    const catCount = {};
    for (const pr of state.realEstate) { const cat = pr.cat || 'residential'; catCount[pr.region + '|' + cat] = (catCount[pr.region + '|' + cat] || 0) + 1; }
    for (const pr of state.realEstate) {
      const r = state.regions.find(x => x.id === pr.region);
      const T = RE_TYPES[pr.type] || RE_TYPES[RE_ALIAS[pr.type] || 'house'];
      if (pr.phase === 'building') {
        pr.buildTicksRemaining = Math.max(0, (pr.buildTicksRemaining || 0) - 1);
        pr.currentValue = (pr.capitalInvested || pr.purchasePrice) * 0.85; // obra a medio terminar vale menos
        pr.rentPerTick = 0; pr.occupancy = 0;
        if (pr.buildTicksRemaining <= 0) {
          pr.phase = 'operating'; pr.occupancy = 0.75;
          if (T.boost && r) { r.landPrice0 *= (1 + T.boost); pushLog(state, 'Megaproyecto terminado en ' + r.name + ': la zona se revaloriza.', 'good'); }
          pushLog(state, 'Obra terminada: ' + T.name + ' en ' + (r ? r.name : pr.region) + ' ya genera renta.', 'good');
        }
        continue;
      }
      const inflMul = state.macro.inflationIndex / (pr.inflAtBuy || 1);
      const cycMul = 1 + (T.cyc || 1) * 0.12 * state.macro.cyclePhase;
      // valor: obra terminada vale ~25% mas que el capital + drift regional
      pr.currentValue = (pr.capitalInvested || pr.purchasePrice) * 1.25 * (r ? r.landPrice / pr.landPriceAtBuy : 1) * inflMul * cycMul * (1 + pr.developmentLevel * 0.3);
      const cat = pr.cat || 'residential';
      const demandUnits = (r ? r.population / 1e6 * r.wealthIndex : 5) * (cat === 'industrial' ? 0.6 : cat === 'commercial' ? 0.9 : cat === 'hospitality' ? 0.5 : cat === 'mega' ? 0.8 : 1.2);
      const supplyUnits = (catCount[pr.region + '|' + cat] || 1);
      pr.occupancy = clamp((0.72 + 0.3 * (demandUnits / supplyUnits - 1) * 0.3) * (1 + 0.15 * state.macro.cyclePhase * (T.cyc || 1) * 0.5), 0.25, 1);
      pr.rentPerTick = pr.currentValue * (T.rentY || 0.095) / 52 * (1 + pr.developmentLevel * 0.2);
    }
  ...
  function maybeEvent(state) {
    ...
    if (state.event.cooldown > 0) { state.event.cooldown--; }
    else if (state.pendingEvents.length < 4 && rngNext(state) < C.EVENT_BASE_PROB) {
      const ev = rollEvent(state);
      if (ev) { ev.uid = uid('ev'); ev.tick = state.tick; state.pendingEvents.push(ev); state.event.cooldown = C.EVENT_COOLDOWN;
        pushLog(state, 'Nueva decisión pendiente: ' + ev.title + '.', 'warn'); }
    }
    // espejo para compatibilidad: el "activo" es el primero de la bandeja
    state.event.active = state.pendingEvents[0] || null;
  }
  ...
  function applyAction(state, action) {
    ...
    switch (A.type) {
      ...
      case 'buyProperty': {
        ...
        const typeId = RE_TYPES[A.propType] ? A.propType : (RE_ALIAS[A.propType] || 'house');
        const T = RE_TYPES[typeId];
        const price = T.base * r.landPrice * state.macro.reIndex;
        ...
        const pr = {
          id: uid('prop'), region: r.id, type: typeId, cat: T.cat, purchasePrice: price, currentValue: price * 0.85,
          phase: 'building', buildTicksRemaining: T.build, capitalInvested: price,
          rentPerTick: 0, occupancy: 0, maintenancePerTick: price * 0.0002,
        ...
        pushLog(state, 'Iniciaste obra: ' + T.name + ' en ' + r.name + (loan ? ' (con hipoteca)' : '') + '. ' + T.build + ' semanas de construccion.', 'info');
        recomputeNetWorth(state);
      ...
      case 'sellProperty': {
        const pr = state.realEstate.find(x => x.id === A.propertyId); if (!pr) return bad('Propiedad no encontrada');
        let net = pr.phase === 'building' ? (pr.capitalInvested || pr.purchasePrice) * 0.8 : pr.currentValue * 0.97;
      ...
      case 'buyClub': {
        if (state.football) return bad('Ya tenes un club.');
        const price = 2200000; // club de 5ta division
        if (pl.cash < price) return bad('Comprar un club de 5ta division cuesta USD ' + price.toLocaleString('en') + '.');
        pl.cash -= price;
        const f = {
          clubName: A.name || 'Union Victoria FC', division: 5, seasonNum: 1, seasonTick: 0, played: 0, pts: 0,
          players: [], coach: { name: fbName(state), quality: 0.3, wage: 900 },
          academy: 0, facilities: 0,
          stadium: { capacity: 4500, ticketPrice: 12, upgradeTicks: 0 },
          fanbase: 6000, brand: 0.05, strength: 0, leaguePos: null, lastResults: '', cashContribution: 0,
        };
        for (let i = 0; i < 16; i++) f.players.push(fbMakePlayer(state, 5, i >= 12));
        state.football = f;
        fbNewSeasonTable(state, f);
        pushLog(state, 'Compraste ' + f.clubName + ' (5ta division inglesa) por USD ' + price.toLocaleString('en') + '. El camino a la elite empieza abajo.', 'good');
        awardMilestone(state, 'fbClub', 'Compraste un club de futbol ingles.');
        recomputeNetWorth(state);
        return { ok: true };
      }
      case 'fbBuyPlayer': {
        const f = state.football; if (!f) return bad('No tenes club.');
        if (!fbWindowOpen(f)) return bad('El mercado de pases esta cerrado (ventanas: inicio de temporada y mitad).');
        const i = f.market.findIndex(p => p.id === A.playerId); if (i < 0) return bad('Jugador no disponible');
        const p = f.market[i];
        if (pl.cash < p.fee) return bad('Fee de transferencia: USD ' + p.fee.toLocaleString('en'));
        if (f.players.length >= 25) return bad('Plantel completo (25).');
        pl.cash -= p.fee; f.fin.fees += p.fee;
        f.market.splice(i, 1);
        const np = Object.assign({}, p); delete np.fee; np.contract = 3; np.morale = 0.75;
        f.players.push(np);
        pushLog(state, 'Fichaste a ' + np.name + ' (' + np.pos + ', nivel ' + np.ability + ') por USD ' + p.fee.toLocaleString('en') + '.', 'good');
        return { ok: true };
      }
      case 'fbSellPlayer': {
        const f = state.football; if (!f) return bad('No tenes club.');
        if (!fbWindowOpen(f)) return bad('El mercado de pases esta cerrado.');
        const i = f.players.findIndex(p => p.id === A.playerId); if (i < 0) return bad('Jugador no encontrado');
        if (f.players.length <= 12) return bad('Plantel minimo: 12 jugadores.');
        const p = f.players[i];
        const fee = Math.round(p.value * 0.9);
        pl.cash += fee; f.fin.sales += fee;
        f.players.splice(i, 1);
        for (const q of f.players) q.morale = clamp(q.morale - (p.ability > 60 ? 0.06 : 0.02), 0.2, 1);
        pushLog(state, 'Vendiste a ' + p.name + ' por USD ' + fee.toLocaleString('en') + '.', 'info');
        return { ok: true, fee };
      }
      case 'fbRenew': {
        const f = state.football; if (!f) return bad('No tenes club.');
        const p = f.players.find(x => x.id === A.playerId); if (!p) return bad('Jugador no encontrado');
        p.contract += 2; p.wage = Math.round(p.wage * 1.3); p.morale = clamp(p.morale + 0.1, 0, 1);
        pushLog(state, 'Renovaste a ' + p.name + ' (+2 temporadas, salario +30%).', 'info');
        return { ok: true };
      }
      case 'fbCoach': {
        const f = state.football; if (!f) return bad('No tenes club.');
        const tier = clamp(fin(A.tier, 0.5), 0.2, 1);
        const cost = Math.round(50000 * tier * tier * 10);
        if (pl.cash < cost) return bad('Contratar este DT cuesta USD ' + cost.toLocaleString('en') + ' (indemnizacion + prima).');
        pl.cash -= cost;
        f.coach = { name: fbName(state), quality: tier, wage: Math.round(600 + tier * 9000) };
        pushLog(state, 'Nuevo DT: ' + f.coach.name + ' (calidad ' + Math.round(tier * 100) + ').', 'good');
        return { ok: true };
      }
      case 'fbAcademy': {
        const f = state.football; if (!f) return bad('No tenes club.');
        if (f.academy >= 5) return bad('Academia al maximo.');
        const cost = 400000 * (f.academy + 1);
        if (pl.cash < cost) return bad('Costo USD ' + cost.toLocaleString('en'));
        pl.cash -= cost; f.academy++;
        pushLog(state, 'Academia nivel ' + f.academy + ': mas juveniles y de mayor potencial.', 'good');
        return { ok: true };
      }
      case 'fbFacilities': {
        const f = state.football; if (!f) return bad('No tenes club.');
        if (f.facilities >= 5) return bad('Instalaciones al maximo.');
        const cost = 600000 * (f.facilities + 1);
        if (pl.cash < cost) return bad('Costo USD ' + cost.toLocaleString('en'));
        pl.cash -= cost; f.facilities++;
        pushLog(state, 'Instalaciones nivel ' + f.facilities + ': mejor desarrollo de jugadores.', 'good');
        return { ok: true };
      }
      case 'fbStadium': {
        const f = state.football; if (!f) return bad('No tenes club.');
        if (f.stadium.upgradeTicks > 0) return bad('Ya hay una obra en curso.');
        const cost = Math.round(f.stadium.capacity * 350);
        if (pl.cash < cost) return bad('Ampliar el estadio cuesta USD ' + cost.toLocaleString('en'));
        pl.cash -= cost; f.stadium.upgradeTicks = 20;
        pushLog(state, 'Obra iniciada: ampliacion del estadio (+35% capacidad, 20 semanas).', 'info');
        return { ok: true };
      }
      case 'fbTicket': {
        const f = state.football; if (!f) return bad('No tenes club.');
        f.stadium.ticketPrice = clamp(fin(A.price, 12), 2, 300);
        return { ok: true };
      }
      case 'donate': {
        const n = (state.nations || []).find(x => x.id === A.countryId); if (!n) return bad('Pais no encontrado');
        const amt = fin(A.amount, 0); if (amt <= 0) return bad('Monto invalido');
        if (pl.cash < amt) return bad('No tenes ese efectivo.');
        pl.cash -= amt; n.received += amt;
        pl.philanthropy = (pl.philanthropy || 0) + amt;
        // impacto por dolar: mucho mayor en paises pobres; rendimientos decrecientes con lo ya recibido
        const scale = n.gdpPerCapita * n.population * 0.02;
        const eff = amt / Math.max(scale, 1) * (1 / (1 + n.received / Math.max(scale * 3, 1)));
        const pts = clamp(eff * 100, 0, 15);
        const area = A.area || 'general';
        if (area === 'salud') n._pendH = (n._pendH || 0) + pts;
        else if (area === 'educacion') n._pendE = (n._pendE || 0) + pts;
        else if (area === 'pobreza') n._pendP = (n._pendP || 0) + pts * 0.7;
        else { n._pendH = (n._pendH || 0) + pts * 0.3; n._pendE = (n._pendE || 0) + pts * 0.3; n._pendP = (n._pendP || 0) + pts * 0.2; n._pendG = (n._pendG || 0) + eff * 0.05; }
        // beneficio suave de reputacion: marca de todas tus empresas (capeado)
        for (const co of state.companies) co.brandStrength = clamp(co.brandStrength + Math.min(0.02, amt / 5e8), 0, 1);
        pushLog(state, 'Donaste USD ' + Math.round(amt).toLocaleString('en') + ' a ' + n.name + ' (' + area + '). Tu legado crece.', 'good');
        awardMilestone(state, 'firstDonate', 'Primera donacion internacional.');
        recomputeNetWorth(state);
        return { ok: true };
      }
      case 'chooseEvent': {
        if (!state.event.active) return bad('No hay evento activo');
        return resolveEvent(state, A.choiceIndex, A.eventUid);
      }
      default:
  ...
  function resolveEvent(state, idx, evUid) {
    if (!state.pendingEvents) state.pendingEvents = [];
    let qi = evUid ? state.pendingEvents.findIndex(e => e.uid === evUid) : 0;
    if (qi < 0 || !state.pendingEvents[qi]) return bad('No hay evento');
    const active = state.pendingEvents[qi];
    const def = EVENTS.find(e => e.id === active.id);
    const choice = def.choices[idx]; if (!choice) return bad('Opcion invalida');
    choice.apply(state);
    pushLog(state, 'Decision tomada (' + active.title + '): ' + choice.label + '.', 'info');
    state.pendingEvents.splice(qi, 1);
    state.event.active = state.pendingEvents[0] || null;
    recomputeNetWorth(state);
    return { ok: true };
  }
  ...
  // ================================================== CLUB DE FUTBOL (Bloque 6)
  // 1 temporada = 42 ticks (38 fechas + 4 de receso). Divisiones 5 (semi-amateur) a 1 (elite).
  const FB = {
    SEASON: 42, MATCHES: 38,
    TV: { 1: 1500000, 2: 350000, 3: 90000, 4: 30000, 5: 9000 },          // TV/premios por semana
    PRIZE: { 1: 40e6, 2: 9e6, 3: 2.5e6, 4: 800000, 5: 250000 },          // premio fin de temporada (1er puesto; escala por posicion)
    ABILITY: { 1: [68, 88], 2: [58, 76], 3: [48, 66], 4: [38, 56], 5: [30, 46] }, // rango de nivel por division
    RIVALS: ['Ironbridge United','Blackmoor City','Redhaven FC','Grimsdale Athletic','Kingsport Rovers','Ashford Town','Milldale Wanderers','Stonecroft FC','Harborview FC','Oldbury Albion','Westfen United','Northgate FC','Silverton City','Foxhall Rangers','Eastmere Town','Dunmoor FC','Claybrook FC','Wolverdale FC'],
    FIRST: ['Jack','Harry','Ollie','Tom','Alfie','Charlie','Liam','Mason','Kai','Theo','Leo','Finn','Reece','Callum','Dylan','Owen','Luca','Ethan'],
    LAST: ['Whitfield','Barrow','Chapman','Holloway','Sutton','Byrne','Farrell','Winslow','Hartley','Mercer','Ashby','Prescott','Dunmore','Kettle','Rowntree','Ellison','Granger','Moss'],
    POS: ['POR','DEF','DEF','DEF','DEF','MED','MED','MED','MED','DEL','DEL'],
  };
  function fbName(state) { return pick(state, FB.FIRST) + ' ' + pick(state, FB.LAST); }
  function fbPlayerValue(p) {
    // valor de mercado: nivel^2.6 escalado; los jovenes con potencial valen mas
    const base = Math.pow(p.ability, 2.6) * 3;
    const youth = p.age < 23 ? 1 + (p.potential - p.ability) / 60 : 1;
    const ageMul = p.age >= 31 ? Math.max(0.3, 1 - (p.age - 30) * 0.15) : 1;
    return Math.round(base * youth * ageMul);
  }
  function fbMakePlayer(state, div, young) {
    const [lo, hi] = FB.ABILITY[div];
    const age = young ? rngInt(state, 17, 21) : rngInt(state, 20, 32);
    const ability = Math.round(rngRange(state, lo, hi) * (young ? 0.8 : 1));
    const potential = Math.min(95, ability + (age < 23 ? rngInt(state, 5, 25) : rngInt(state, 0, 4)));
    const p = { id: uid('pl'), name: fbName(state), pos: pick(state, FB.POS), age, ability, potential, wage: 0, contract: rngInt(state, 2, 4), morale: 0.7 };
    p.value = fbPlayerValue(p);
    p.wage = Math.round(p.value * 0.004 + 150); // salario semanal ~ valor
    return p;
  }
  function fbTeamStrength(f) {
    const sorted = f.players.slice().sort((a, b) => b.ability - a.ability).slice(0, 11);
    if (!sorted.length) return 5;
    const avg = sorted.reduce((s, p) => s + p.ability, 0) / sorted.length;
    const moraleAvg = f.players.reduce((s, p) => s + p.morale, 0) / Math.max(f.players.length, 1);
    return avg * (0.8 + 0.4 * f.coach.quality) * (0.88 + 0.12 * moraleAvg) * (1 + 0.03 * f.facilities);
  }
  function fbNewSeasonTable(state, f) {
    const [lo, hi] = FB.ABILITY[f.division];
    f.table = [];
    for (let i = 0; i < 9; i++) f.table.push({ name: FB.RIVALS[(i + f.seasonNum * 3) % FB.RIVALS.length], strength: rngRange(state, lo + 3, hi + 6), pts: 0 });
    f.pts = 0; f.seasonTick = 0; f.played = 0;
    f.fin = { tv: 0, tickets: 0, merch: 0, sponsor: 0, sales: 0, prizes: 0, wages: 0, staff: 0, fees: 0, maint: 0 };
    fbRefreshMarket(state, f);
  }
  function fbRefreshMarket(state, f) {
    f.market = [];
    for (let i = 0; i < 8; i++) {
      const div = clamp(f.division + (i < 3 ? -1 : 0), 1, 5); // algunos de division superior
      const p = fbMakePlayer(state, div, i >= 5);
      p.fee = Math.round(p.value * 1.25);
      f.market.push(p);
    }
  }
  function fbWindowOpen(f) { const t = f.seasonTick; return (t >= 0 && t < 6) || (t >= 20 && t < 26); }

  function stepFootball(state) {
    const f = state.football; if (!f) return;
    const pl = state.player;
    f.seasonTick++;
    const strength = fbTeamStrength(f);
    f.strength = Math.round(strength);
    let income = 0, expenses = 0;
    // === partido semanal ===
    if (f.played < FB.MATCHES) {
      const rival = f.table[f.played % f.table.length];
      const diff = (strength - rival.strength) / 18;
      const roll = rngNext(state) + diff * 0.35;
      let res;
      if (roll > 0.62) { f.pts += 3; res = 'G'; } else if (roll > 0.42) { f.pts += 1; res = 'E'; } else res = 'P';
      f.lastResults = ((f.lastResults || '') + res).slice(-8);
      f.played++;
      // rivales suman puntos estadisticamente
      for (const r of f.table) { const rr = rngNext(state) + (r.strength - (FB.ABILITY[f.division][0] + FB.ABILITY[f.division][1]) / 2) / 40; if (rr > 0.6) r.pts += 3; else if (rr > 0.4) r.pts += 1; }
      // moral e hinchada siguen los resultados
      const winBoost = res === 'G' ? 0.02 : res === 'P' ? -0.02 : 0;
      for (const p of f.players) p.morale = clamp(p.morale + winBoost, 0.2, 1);
      f.fanbase = Math.max(2000, Math.round(f.fanbase * (1 + winBoost * 0.5)));
      // dia de partido: entradas (cada 2 fechas es local)
      if (f.played % 2 === 0) {
        const interest = clamp(f.fanbase * (0.5 + strength / 200), 0, f.stadium.capacity * 3);
        const att = Math.min(f.stadium.capacity, interest * clamp(1.3 - f.stadium.ticketPrice / (12 + f.division * 6), 0.2, 1));
        f.lastAttendance = Math.round(att);
        income += att * f.stadium.ticketPrice; f.fin.tickets += att * f.stadium.ticketPrice;
      }
    }
    // === ingresos semanales ===
    const tv = FB.TV[f.division]; income += tv; f.fin.tv += tv;
    const merch = f.fanbase * 0.35 * (1 + f.brand); income += merch; f.fin.merch += merch;
    const sponsor = tv * 0.25 * (1 + f.brand * 2); income += sponsor; f.fin.sponsor += sponsor;
    // === gastos semanales ===
    const wages = f.players.reduce((s, p) => s + p.wage, 0); expenses += wages; f.fin.wages += wages;
    const staff = f.coach.wage + 400 * (1 + f.academy + f.facilities); expenses += staff; f.fin.staff += staff;
    const maint = f.stadium.capacity * 0.9 + (f.academy + f.facilities) * 600; expenses += maint; f.fin.maint += maint;
    // obra del estadio
    if (f.stadium.upgradeTicks > 0) { f.stadium.upgradeTicks--; if (f.stadium.upgradeTicks === 0) { f.stadium.capacity = Math.round(f.stadium.capacity * 1.35); pushLog(state, 'Ampliacion del estadio terminada: capacidad ' + f.stadium.capacity.toLocaleString('en') + '.', 'good'); } }
    f.cashContribution = income - expenses;
    pl.cash += f.cashContribution; state._quarterProfit += f.cashContribution;
    // FFP: gastar sostenidamente el doble de lo que ingresa trae sancion
    if (expenses > income * 2 && expenses > 20000) { f.ffpStreak = (f.ffpStreak || 0) + 1; } else f.ffpStreak = 0;
    if (f.ffpStreak === 12) { const fine = Math.round(expenses * 6); pl.cash -= fine; f.ffpStreak = 0; pushLog(state, 'Sancion de fair play financiero: USD ' + fine.toLocaleString('en') + '. Equilibra los gastos del club.', 'danger'); }
    // desarrollo semanal de jovenes / declive de veteranos
    for (const p of f.players) {
      if (p.age < 23 && p.ability < p.potential && rngNext(state) < 0.1 * (1 + f.facilities * 0.3)) p.ability = Math.min(p.potential, p.ability + 1);
      p.value = fbPlayerValue(p);
    }
    // === fin de temporada ===
    if (f.seasonTick >= FB.SEASON) {
      const table = f.table.map(r => r.pts).concat([f.pts]).sort((a, b) => b - a);
      const pos = table.indexOf(f.pts) + 1;
      f.leaguePos = pos;
      const prize = Math.round((FB.PRIZE[f.division] || 0) * Math.max(0.05, 1 - (pos - 1) * 0.12));
      pl.cash += prize; f.fin.prizes += prize; state._quarterProfit += prize;
      let msg = 'Temporada ' + f.seasonNum + ' (Div ' + f.division + '): terminaste ' + pos + 'º de 10. Premio USD ' + prize.toLocaleString('en') + '.';
      if (pos <= 2 && f.division > 1) { f.division--; f.fanbase = Math.round(f.fanbase * 1.6); f.brand = clamp(f.brand + 0.1, 0, 1); msg += ' ¡ASCENSO a la division ' + f.division + '!'; awardMilestone(state, 'fbPromo' + f.division, 'Ascenso a la division ' + f.division + '.'); }
      else if (pos >= 9 && f.division < 5) { f.division++; f.fanbase = Math.round(f.fanbase * 0.75); msg += ' Descenso a la division ' + f.division + '.'; }
      if (pos === 1 && f.division === 1) { msg += ' ¡CAMPEON DE LA PRIMERA DIVISION!'; awardMilestone(state, 'fbChamp', 'Campeon de la primera division inglesa.'); f.brand = clamp(f.brand + 0.2, 0, 1); }
      pushLog(state, msg, pos <= 2 ? 'good' : 'info');
      // envejecer, contratos, academia
      for (let i = f.players.length - 1; i >= 0; i--) {
        const p = f.players[i]; p.age++; p.contract--;
        if (p.age >= 31) p.ability = Math.max(20, p.ability - rngInt(state, 1, 3));
        if (p.contract <= 0) { pushLog(state, p.name + ' se fue libre (contrato vencido).', 'warn'); f.players.splice(i, 1); }
        else p.value = fbPlayerValue(p);
      }
      if (f.academy > 0 && rngNext(state) < 0.25 * f.academy) {
        const y = fbMakePlayer(state, f.division, true); y.potential = Math.min(95, y.potential + f.academy * 3);
        f.players.push(y); pushLog(state, 'La academia produjo un juvenil: ' + y.name + ' (potencial ' + y.potential + ').', 'good');
      }
      f.seasonNum++;
      fbNewSeasonTable(state, f);
    } else if (fbWindowOpen(f) && f.seasonTick % 6 === 0) fbRefreshMarket(state, f);
  }
  function footballValue(f) {
    if (!f) return 0;
    const squad = f.players.reduce((s, p) => s + p.value, 0);
    return squad + f.stadium.capacity * 250 + f.fanbase * 20 * (1 + f.brand);
  }

  // ================================================== NACIONES (Bloque 7)
  function buildNations() {
    const mk = (id, name, gpc, pop, pov, he, ed) => ({ id, name, gdpPerCapita: gpc, population: pop, poverty: pov, health: he, education: ed, development: Math.round((he + ed) / 2 - pov * 0.3), received: 0 });
    return [
      mk('nor', 'Noruega', 82000, 5.5e6, 1, 95, 96), mk('jpn', 'Japon', 34000, 124e6, 2, 94, 92),
      mk('chl', 'Chile', 16000, 19e6, 9, 80, 78), mk('mex', 'Mexico', 11000, 128e6, 36, 72, 68),
      mk('bra', 'Brasil', 9200, 214e6, 29, 70, 66), mk('ind', 'India', 2400, 1400e6, 12, 60, 55),
      mk('ken', 'Kenia', 2100, 54e6, 36, 52, 50), mk('hti', 'Haiti', 1700, 11.5e6, 59, 40, 38),
      mk('eth', 'Etiopia', 1000, 120e6, 27, 44, 36), mk('ner', 'Niger', 590, 26e6, 41, 35, 22),
    ];
  }
  function stepNations(state) {
    if (!state.nations) { state.nations = buildNations(); return; }
    for (const n of state.nations) {
      // deriva natural lenta: los desarrollados crecen algo, los pobres se estancan
      const drift = n.development > 60 ? 0.004 : n.development > 40 ? 0.002 : 0.0005;
      n.gdpPerCapita *= (1 + drift / 52 * 10);
      n.health = clamp(n.health + drift * 0.3, 0, 100); n.education = clamp(n.education + drift * 0.3, 0, 100);
      // el efecto de las donaciones pendientes se aplica gradual
      if (n._pendH) { const d = Math.min(n._pendH, 0.15); n.health = clamp(n.health + d, 0, 100); n._pendH -= d; }
      if (n._pendE) { const d = Math.min(n._pendE, 0.15); n.education = clamp(n.education + d, 0, 100); n._pendE -= d; }
      if (n._pendP) { const d = Math.min(n._pendP, 0.1); n.poverty = clamp(n.poverty - d, 0.5, 80); n._pendP -= d; }
      if (n._pendG) { const d = Math.min(n._pendG, 0.002); n.gdpPerCapita *= (1 + d); n._pendG -= d; }
      n.development = Math.round(clamp((n.health + n.education) / 2 - n.poverty * 0.3 + Math.log10(n.gdpPerCapita) * 4, 0, 100));
    }
    // hito de legado
    if (state.nations.filter(n => n.received > 0 && n.development >= 70).length >= 3) awardMilestone(state, 'legacy3', 'Legado: 3 paises ayudados superaron desarrollo 70.');
  }
  ...
  return {
    ...
    bondRateFor, competitorHealth, tenderTerms, RE_TYPES, FB, footballValue, fbTeamStrength, fbWindowOpen,
  };
});
